...
def Detect(file_path):
    ...
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            roi = roi.astype('float32') / 255  # Normalize the ROI
...
def show_Detect_button(file_path):
    ...
    detect_b.configure(background="#364156", foreground='white', font=('ariel', 10, 'bold'))
...
upload.configure(background="white", foreground='#364156', font=('ariel', 20, 'bold'))
upload.pack(side='bottom', pady=50)
sign_image.pack(side='bottom', expand=True)
label1.pack(side='bottom', expand=True)
heading = Label(top, text="Emotion Detector", pady=20, font=('ariel', 25, 'bold'))
